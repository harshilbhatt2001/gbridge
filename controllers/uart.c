...
static int uart_init(struct controller *ctrl) { return 0; }
...
static int uart_write(struct connection *conn, void *data, size_t len) {
  ...
  for (remaining = len, offset = 0, written = 0; remaining;
...
static int _uart_read(struct uart_controller *ctrl, void *data, size_t len) {
  ...
  for (remaining = len, offset = 0, recvd = 0; remaining;
...
static int uart_read(struct interface *intf, uint16_t *cport_id, void *data,
                     size_t len) {
  int ret;
  uint8_t *p_data = data;
